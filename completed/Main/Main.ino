  ...
  for (int i = 0; i < NUM_LEDS; i++) { // one rotation
    for (int k = 0 ; k < repeat; k++) {
      for (int j = 0; j < NUM_LEDS / repeat; j += 1) { //this loop jumps to the start of each new color segment by that length
        ...
        leds[(i + j + k * NUM_LEDS / repeat) % NUM_LEDS] = hueColors[(int)map(j, 0, NUM_LEDS / repeat, 0, 360)];
      }
    }
    FastLED.show();
    delay(d);
...
double lerp(int from, int to, double percent) {
  return to+ (to - from) * percent;
}


void smoothFade(CRGB toColor, int d) {
  CRGB oldLeds[NUM_LEDS] = leds;
//  for (int i = 0; i < NUM_LEDS; i++) {
//    oldLeds[i] = leds[i];
//  }
  ...
  for (double i = 0; i < 1; i += 0.01) {
    for (int j = 0; j < NUM_LEDS; j++) {
//      CRGB oldColor = oldLeds[i];
//      CRGB newColor = CRGB(
//                  lerp(oldColor.red, toColor.red, i),
//                  lerp(oldColor.green, toColor.green, i),
//                  lerp(oldColor.blue, toColor.blue, i)
//                ); 
//      leds[i] = newColor;
    }
  }
